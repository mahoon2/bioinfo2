...
def main():
    ...
    pam = pd.read_csv('pam250.mat.csv', index_col=0)
    seq1, seq2 = [line.strip() for line in infile.readlines() if line.strip()]
    ...
    for i in tqdm(range(1, len(seq1) + 1)):
        ...
        for j in range(1, len(seq2) + 1):
            ...
            temp = [
                ...
                (i - 1, j - 1, ret[i - 1][j - 1] + pam.loc[seq1aa, seq2aa]),
            ]
            temp.sort(key=lambda l: l[2], reverse=True)
            ret[i][j] = temp[0][2]
            backtrack[i][j] = (temp[0][0], temp[0][1])

    i, j = np.argmax(ret) // (len(seq2) + 1), np.argmax(ret) % (len(seq2) + 1)
    outfile.write(str(ret[i][j]) + '\n')

    out1 = ''
    out2 = ''
    while (i > 0 or j > 0) and ret[i][j] > 0:
